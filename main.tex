...
We note that \(F_{TAP}\) is expressed in terms of the magnetizations \(\mathbf{m}^v, \mathbf{m}^h\) which are given by eq. \eqref{eq:act_vis},\eqref{eq:act_hid}. Given the approximation to the free energy \eqref{eq:f_tap} the optimization problem over the log-likelihood is greatly simplified: the average over the training samples <> is unchanged while the intractable average over the model distribution <> is substituted by the minimization of the TAP free energy
